...
function DialogueTree( dialogueJSON ){
  ...
  let parseBody = function(bodyText){
    ...
    for(let k in bodySplitToLines){
      let lineSeparators = ["[[", "]]", "<<", ">>", "{{", "}}"];
      let splitLine = bodySplitToLines[k].splitBySeparators(lineSeparators, true, true)
      let linkStart = false;
      ...
      for(let i in splitLine){
        ...
        if(linkStart){
          lineHasLink = true;
          let splitLink = st.splitBySeparators("|")
          let linkObj = {
            ...
            get displayText(){
              let newDisplayText = '';
              let lineSeparators = ["${", "}"]
              let splitDisplayText = this.rawDisplayText.splitBySeparators(lineSeparators, true, true)
              let evalStart = false;
    ...
    let trimTrailingChars = function(s, charToTrim) {
    ...
    rawBody = trimTrailingChars(rawBody, "\n")
    return {links, rawBody, commands}
  }
  
  let splitBySeparators = function(myStr, array_of_separators, removeEmptyStrings, leaveSeparatorsIn){
    var new_str = myStr;
