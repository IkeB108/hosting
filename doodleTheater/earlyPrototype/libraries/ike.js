...
function handleFile(file){
  ...
  I think this should be able to handle multiple files?
  It may just get called once for each file
  */
...
function ImageToString(p5image){
  /*given a p5 image or p5 graphics object, returns a string
  containing base64 encoded image data*/
  p5image.loadPixels();
  return p5image.canvas.toDataURL()
}

function StringToImage(str){
  /*
  given a string made with ImageToString(),
  returns a p5 image created from the data.
  It will take about 2 frames before the image is fully loaded,
  so you can't draw the image right away.
  loadJSON() must be called in preload(), and StringToImage() must be called after preload() (in setup() or draw())
  */
...
function canvasPosition(element, canvas, xarg, yarg){
  ...
  element.position(canvas.position().x + myCanvas.width/2, canvas.position().y + myCanvas.height/2)
